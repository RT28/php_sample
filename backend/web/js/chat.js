...
function onMessageReceived(msg) {
    msg = JSON.parse(msg);
    $('.chat-body').append("<div class=\"received-message\">"+msg.message+"</div>");
}
...
function onChatUnitClick() {
    ...
    var dom = getChatWindowHtml(id, name, role, isOnline);
...
function getChatWindowHtml(toId, toName, toRole, isOnline) {
    ...
    '<div class="chat-window panel panel-default" id="chat-' + toId + '-' + toRole + '">' + 
        '<div class="chat-window-inner">' +
            '<div class="panel-heading panel-heading-chat-title" data-studentid="' + toId + '">'  + 
                    '<img src="images/user-1.jpg" alt="' + toName +'" class="chat-title-image"/>' +
                    toName  +
            ...
            '<div id="chat-container' + toId + '" class="chat-container">' +
                '<div class="panel-body chat-body" id="chat-body'+ toId +'"></div>' +
                ...
                '<input type="hidden" name="to_role" id="to_role'+ toId +'" value="' + toRole + '"/>' +
            '</div>' +        
        '</div>' +
    '</div>';
}
