...
class UniversityController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        if(isset($_POST['partner'])) {
            $models = $models->where(['=', 'is_partner', 1]);
        }
        ...
        $universityTotalCount = University::find()->count();
		...
		if(Yii::$app->request->isAjax) {
			...
			*********************************/
            if (isset($_POST['searchbykeyword'])) {
                ...
                /*university with country search keyword : author: roan----------------------------------*/
            ...
            if(isset($_POST['partner'])) {
             $query = $query->andWhere(['=', 'is_partner', '1']);
        }
        $query = $query->groupBy(['university.id']);
        $ajaxModels = $query->orderBy(['university.name' => 'ASC']);
		$count = $query->count();
		...
		$pages = new Pagination(['totalCount' => $count,'defaultPageSize' => 21,'forcePageParam' => false,'page'=>$currpage,]);
			...
			$ajaxModels = $ajaxModels->offset($pages->offset)
				->limit($pages->limit)
				->all();
