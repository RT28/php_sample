...
use partner\modules\agency\models\PartnerTrainerSearch; 
...
class TrainerController extends Controller
{
    ...
    {
		Yii::$app->view->params['activeTab'] = 'trainer';
		 
        $searchModel = new PartnerTrainerSearch();
    ...
    public function actionCreate()
    {
        ...
        if(Yii::$app->request->post()) {
			...
			if(empty($exists)) {
				...
				$partnerLogin->role_id = Roles::ROLE_TRAINER;
            ...
            if(!empty($model->languages)){
                $model->languages = implode(',',$model->languages);
						...
						$model->role_id = Roles::ROLE_TRAINER;							
						...
						if($model->save()) {
								...
								Yii::$app->getSession()->setFlash('Success', 'Trainer has been created successfully.'); 
							} else {
	...
	public function actionCreatelogin($id)
    {
	  $id = Commondata::encrypt_decrypt('decrypt', $id);
		...
		$data = array('user' => $to, 'user_name' => $user_name, 'link' => $link);		
		...
		return $this->redirect('?r=agency/trainer/index');  
    } 
