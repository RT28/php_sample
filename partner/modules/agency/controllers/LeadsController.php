...
class LeadsController extends \yii\web\Controller
{
    ...
    public function actionIndex() { 
        ...
        $students_new = User::find()->innerJoin('student_consultant_relation', '`user_login`.`id` = `student_consultant_relation`.`student_id` AND `user_login`.`follow_status` = 0 AND `student_consultant_relation`.`agency_id` = "'.$id.'" ' )->all();
        $studentnew =  count($students_new);

        $students_active = User::find()->innerJoin('student_consultant_relation', '`user_login`.`id` = `student_consultant_relation`.`student_id` AND `user_login`.`follow_status` = 1 AND `student_consultant_relation`.`agency_id` = "'.$id.'" ' )->all();
        $studentactive =  count($students_active);

         $students_inactive = User::find()->innerJoin('student_consultant_relation', '`user_login`.`id` = `student_consultant_relation`.`student_id` AND `user_login`.`follow_status` = 2 AND `student_consultant_relation`.`agency_id` = "'.$id.'" ' )->all();
         $studentinactive =  count($students_inactive);

         $students_accesssend = User::find()->innerJoin('student_consultant_relation', '`user_login`.`id` = `student_consultant_relation`.`student_id` AND `user_login`.`follow_status` = 3 AND `student_consultant_relation`.`agency_id` = "'.$id.'" ' )->all();
         $studentaccesssend =  count($students_accesssend);

         $students_subscribed = User::find()->innerJoin('student_consultant_relation', '`user_login`.`id` = `student_consultant_relation`.`student_id` AND `user_login`.`follow_status` = 4 AND `student_consultant_relation`.`agency_id` = "'.$id.'" ' )->all();
         $studentsubscribed =  count($students_subscribed);

         $students_closed = User::find()->innerJoin('student_consultant_relation', '`user_login`.`id` = `student_consultant_relation`.`student_id` AND `user_login`.`follow_status` = 6 AND `student_consultant_relation`.`agency_id` = "'.$id.'" ' )->all();
         ...
         $rows_today = (new \yii\db\Query())
                    ...
                    $students_today = User::find()->innerJoin('student_consultant_relation', '`user_login`.`id` = `student_consultant_relation`.`student_id` AND `student_consultant_relation`.`agency_id` = "'.$id.'" '.$check_condition.'')->all();
		...
		/*to set all leads as read*/         
           ...
           ->update('agency', ['leads_count' => 0], ['partner_login_id' => Yii::$app->user->identity->id])
           ->execute(); 
