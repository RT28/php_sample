...
use partner\modules\employee\models\LeadsSearch;
use frontend\models\StudentShortlistedCourse;
use common\models\StudentFavouriteUniversities;  
use partner\modules\employee\models\TasksSearch;
...
class LeadsController extends \yii\web\Controller
{
    ...
    public function actionIndex() { 
        ...
        $students_new = User::find()->innerJoin('student_partneremployee_relation', '`user_login`.`id` = `student_partneremployee_relation`.`student_id` AND `user_login`.`follow_status` = 0 AND `student_partneremployee_relation`.`parent_employee_id` = "'.$id.'" ' )->all();
        $studentnew =  count($students_new);

        $students_active = User::find()->innerJoin('student_partneremployee_relation', '`user_login`.`id` = `student_partneremployee_relation`.`student_id` AND `user_login`.`follow_status` = 1 AND `student_partneremployee_relation`.`parent_employee_id` = "'.$id.'" ' )->all();
        $studentactive =  count($students_active);

         $students_inactive = User::find()->innerJoin('student_partneremployee_relation', '`user_login`.`id` = `student_partneremployee_relation`.`student_id` AND `user_login`.`follow_status` = 2 AND `student_partneremployee_relation`.`parent_employee_id` = "'.$id.'" ' )->all();
         $studentinactive =  count($students_inactive);

         $students_accesssend = User::find()->innerJoin('student_partneremployee_relation', '`user_login`.`id` = `student_partneremployee_relation`.`student_id` AND `user_login`.`follow_status` = 3 AND `student_partneremployee_relation`.`parent_employee_id` = "'.$id.'" ' )->all();
         $studentaccesssend =  count($students_accesssend);

         $students_subscribed = User::find()->innerJoin('student_partneremployee_relation', '`user_login`.`id` = `student_partneremployee_relation`.`student_id` AND `user_login`.`follow_status` = 4 AND `student_partneremployee_relation`.`parent_employee_id` = "'.$id.'" ' )->all();
         $studentsubscribed =  count($students_subscribed);

         $students_closed = User::find()->innerJoin('student_partneremployee_relation', '`user_login`.`id` = `student_partneremployee_relation`.`student_id` AND `user_login`.`follow_status` = 6 AND `student_partneremployee_relation`.`parent_employee_id` = "'.$id.'" ' )->all();
         ...
         $rows_today = (new \yii\db\Query())
                    ...
                    $students_today = User::find()->innerJoin('student_partneremployee_relation', '`user_login`.`id` = `student_partneremployee_relation`.`student_id` AND `student_partneremployee_relation`.`parent_employee_id` = "'.$id.'" '.$check_condition.'')->all();
  ...
  public function actionAssignConsultant() {
	  ...
	  $parent_employee_id = Yii::$app->user->identity->id;
	 ...
	 $id = $_REQUEST['id'];  
	  ...
	  ->where(['parent_partner_login_id' => $parent_employee_id])
       ...
       $consultant_name = ArrayHelper::map($allConsultants,'consultant_id','first_name');
		...
		$name = $model->first_name;
			   ...
			   if(isset($consultant)){
				...
				if(!empty($consultant->parent_employee_id)){
					$assignedAgency = Agency::findOne(['partner_login_id'=>$consultant->parent_employee_id]); 
