...
namespace partner\modules\employee\models;
...
class LeadsSearch extends Student
{
    ...
    $majors_preference, $begin,$package_type,$srmdetails,$package,$packagedetails,$consultant,$employee,$srm,$consultantdetails,$leadsfollowupdetails,$course , $consultant_id ,  $status;
    public function rules()
    {
        return [
            [[ 'city', 'state', 'country', 'created_by', 'updated_by'], 'integer'],
            [['id','first_name', 'last_name', 'date_of_birth', 'gender', 'address', 'street', 'pincode', 'email',  'phone',  'created_at', 'updated_at' ,'srmdetails','srm','package','consultant','employee','packagedetails','consultantdetails','leadsfollowupdetails','consultant_id','status'], 'safe'],
        ];
    }
    ...
    public function search($params)
    {
        ...
        //$query->where(['student_consultant_relation.agency_id'=>1]) ;
        $query->where(['student_partneremployee_relation.parent_employee_id'=>$id]);
