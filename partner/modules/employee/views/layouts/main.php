...
if(Yii::$app->user->identity->role_id == Roles::ROLE_EMPLOYEE) {
	...
	$Employee = PartnerEmployee::find()->where(['AND',  ['=', 'partner_login_id', $userId]])->one();
...
<input type="hidden" id="from_role" value="<?= Roles::ROLE_EMPLOYEE; ?>">
<!-- WRAPPER-->
...
<?php
 $src = ""; 
    ...
    $src = './noprofile.gif'; 
   $is_profile = 0;
	if(is_dir('./uploads/employee/' . $userId . '/profile_photo')) {
        $cover_photo_path = "./uploads/employee/".$userId."/profile_photo/consultant_image_228X228";
...
<div class="profile-pic"> <img id="target_logo" class="student-profile-photo" src="<?= $src; ?>" alt="<?= $Employee->first_name;?>"/> 
</div>

<form id="ppForm" name="myForm1" method="post" action="?r=employee/employee/saveprofileajax" enctype="multipart/form-data">
<a style="cursor: pointer;" id="uploadlink">Change Profile Picture</a>
<?php if($is_profile==1){ ?>
...
<li><a href="?r=employee/employee/update" class="<?= ($activeTab == 'profile') ? 'active' : '';?>">My Profile</a></li> 
<li><a href="?r=employee/students/index" class="<?= ($activeTab == 'students') ? 'active' : '';?>">My Students</a></li> 
<!-- <li><a href="?r=employee/leads/index&status=0" class="<?= ($activeTab == 'leads') ? 'active' : '';?>">Leads</a></li> 
 -->    
</ul>
...
$(document).ready(function()
{
 ...
 $('#remove_pro_pic').on('click',function(evt){
  ...
  $('#remove_approve').on('click',function(evt){
          $.ajax({
            url: '?r=employee/employee/removeprofileajax',
            method: 'POST',
