...
use partner\modules\consultant\models\LeadFollowupSearch;
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if(isset($_POST['reason_code']) AND $_POST['reason_code']!=0){
                ...
                if($model->status ==3){
                        $id = Commondata::encrypt_decrypt('encrypt', $model->student_id);
                        $link = "index.php?r=consultant/students/assign-package&id=". $id;
                        return $this->redirect($link);
                    }
                }
                if($model->status ==1){
                CalendarEvents::assignLeadcalender($model, $model->consultant_id, Roles::ROLE_CONSULTANT,CalendarEvents::TYPE_FOLLOWUP,CalendarEvents::MODE_PHONE);
                }
                if($model->status ==5){
                return $this->redirect(['leads/index']);
                }
                else {
                    $link_redirect = "index.php?r=consultant/leads/index&status=".$model->status; 
