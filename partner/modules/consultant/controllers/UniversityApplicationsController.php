...
use partner\modules\consultant\models\StudentUniversityApplicationSearch;
use yii\web\Controller;
use yii\web\NotFoundHttpException;
use yii\filters\VerbFilter;
use common\components\AdmissionWorkflow;
use common\components\Roles;
use common\components\ConnectionSettnings; 
...
class UniversityApplicationsController extends Controller
{
      ...
      public function behaviors()
    {
        return [
            'access' => [
                ...
                'rules' => [   
					[
						'actions' => ['index', 'view',  'update-state'],
						'allow' => true, 
    ...
    private function sendMail($state, $model, $action) {        
        ...
        foreach($roles as $role => $role_details) {
            ...
            $link = "";
            switch($role) {
                case Roles::ROLE_CONSULTANT: $email = PartnerLogin::findOne($model->consultant_id)->email;                
                $link= $base . 'backend/web/index.php?r=university-applications/view&id=' . $model->id;                
                break;
                case Roles::ROLE_STUDENT: $email = Yii::$app->user->identity->email;                
                $link= $base . 'frontend/web/index.php?r=university-applications/view&id=' . $model->id;                
                break;
                case Roles::ROLE_UNIVERSITY:$partner_login = PartnerLogin::find()->where(['=', 'partner_id', $model->university_id])->one();
	 			//$partner = Partner::find() ->where(['=', 'university_id', $model->university_id])->one();
