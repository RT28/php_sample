	...
	data-target="#subconsultantUpdateModal" onclick="loadSubConUpdate(<?= $associate->id; ?>)" ></a>
	 ...
	 <?php if($associate->status ==1){  
	  ...
	  if($associate->status ==0){  
	   ...
	   ?>
		<button  id="status<?php echo $associate->consultant_id; ?>" class="btn  btn-associate-status <?php echo  $class;?> " onclick="connectDisconnectAssociate(<?= $associate->consultant_id; ?>,<?= $associate->student_id; ?>);" value="<?= $associate->status; ?>">
		<?php echo $textstatus;?>
		</button> 
