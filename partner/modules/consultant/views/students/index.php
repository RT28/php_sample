...
$content.='	</div>
	...
	'label' => 'Consultant', 
	'value' => function($searchModel){  
	if(isset($searchModel->is_sub_consultant))	 {
	 $name = $searchModel->consultant->first_name . ' ' .$searchModel->consultant->last_name ;
		if (isset($name)) {
		 ...
		 $name = $searchModel->parentConsultant->first_name . ' ' .$searchModel->parentConsultant->last_name ;
		if (isset($name)) {
			 	return $name; 
		} else{ 
			return 'not assigned'; 
...
if(isset($searchModel->student->phone))	{
	...
	'buttons' => [   
	'addtaskbutton' => function ($url, $searchModel, $key) {
		 
	$path =  '?r=consultant/tasks/create&id='.$searchModel->student_id;
	return  '<a href="#" class="btn btn-blue" data-toggle="modal"  
	data-target="#addtaskModal" onclick=loadTaskAdd("'.$path.'"); >Add Task</a> <br/>';
	},	
	'addtconsultantbutton' => function ($url, $searchModel, $key) { 
		if(isset($searchModel->consultant)){
			$parentConsultantId = Yii::$app->user->identity->id;
			//if(isset($parentConsultantId) && $searchModel->consultant->consultant_id==$parentConsultantId){
		  
		$cpath = '?r=consultant/subconsultant/create/&id='.$searchModel->student_id;
		return  '<a href="#"  data-toggle="modal"  
		data-target="#assignSubConsultantModal" onclick=loadAssignSubConsultant("'.$cpath.'"); >Assign Associate Consultant</a> <br/>'; 
		}
		//}
	},
	'addtemployeebutton' => function ($url, $searchModel, $key) {
		if(isset($searchModel->consultant)){
			$parentConsultantId = Yii::$app->user->identity->id;	
//if(isset($parentConsultantId) && $searchModel->consultant->consultant_id==$parentConsultantId){			 
			 
			$epath = '?r=consultant/assignemployee/create/&id='.$searchModel->student_id;
		return  '<a href="#"  data-toggle="modal"  
			data-target="#assignEmployeeModal" onclick=loadassignemployee("'.$epath.'"); >Assign Trainer/Employee</a> <br/>';
			}
		//}
	},	
	'updateworkstatus' => function ($url, $searchModel, $key) {
		if(isset($searchModel->consultant)){
			$parentConsultantId = Yii::$app->user->identity->id;			 
			
			$associate = StudentConsultantRelation::find()->where(['AND',
			['=','student_id', $searchModel->student_id], 
			['=','consultant_id', $parentConsultantId], 
			['=','is_sub_consultant', 1],['!=','assigned_work_status', 3],
			])->one();	
			
			if($associate){	
			
			return '<a href="#" class="btn btn-blue" data-toggle="modal" 
				data-target="#subconsultantUpdateModal" onclick="loadSubConUpdate('.$associate->id.')" >Update Work Status</a>';
			}
		}
	},
	],
	'template' => '{addtaskbutton}{addtconsultantbutton}{addtemployeebutton}  {updateworkstatus}'], 
	],
	]); ?>
	 <?php endif; ?>
</div>
<div id="assignEmployeeModal" class="modal fade" role="dialog"> 
  <div class="modal-dialog modal-lg"> 
    ...
    <div class="modal-content">
      ...
      <div class="modal-body" id="EmployeePreview" style="height:800px; overflow:scroll;">
        </div> 
...
<div id="assignSubConsultantModal" class="modal fade" role="dialog"> 
  <div class="modal-dialog modal-lg"> 
    <!-- Modal content--> 
    <div class="modal-content">
      <div class="modal-header"> 
      ...
      <div class="modal-body" id="SubConsultantPreview" style="height:800px; overflow:scroll;">
        </div> 
      <div class="modal-footer"> 
        <button type="button" class="btn btn-default" data-dismiss="modal">Close</button>
      </div> 
