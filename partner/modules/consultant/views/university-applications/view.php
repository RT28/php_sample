...
<div class="student-univeristy-application-view">
    ...
    <?php
        ...
        $name = "";
        $user_model;
        if($role == Roles::ROLE_EMPLOYEE || $role == Roles::ROLE_TRAINER) {
            ...
            $user_model = PartnerLogin::findOne($user);
           // $partner = Partner::find()->where(['=', 'partner_id', $user_model->id])->one();
            $temp = University::findOne($user_model->partner_id);
            $name = $temp->name;
