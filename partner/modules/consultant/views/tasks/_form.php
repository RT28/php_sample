...
<?php }else{ ?>
<?= $form->field($model, "student_id")->dropDownList($students, ['prompt' => 'Select Students','onchange'=>'getParent_emails()',]) ?>
<?php }?>
...
<label><input onclick="getParent_emails();" name="Tasks[notified][]"  id="notified3" value="3" type="checkbox" <?php if(in_array(3, $notifiedData)){?> checked="checked"<?php }?> > Parent/Guardian</label>
...
//$('#task_detail').html(response.result); just hidden may be need to enable later
},
...
function getParent_emails() {
if($('#notified3').is(':checked'))
{
  var student_id = $('#tasks-student_id').val();
  if(student_id!==''){
  $.ajax({
            url: '?r=consultant/tasks/getparentemail',
	        method: 'POST',
	        data: {
	            id: student_id,
	        },
            success: function(response, data) {
                response = JSON.parse(response);                
                $('#parent_email').html(response.result); 
            },
            error: function(error) {
                console.log(error);
            }
	...
	$('#parent_email').html('Select Student');
}
}else
{
 $('#parent_email').html(''); 
