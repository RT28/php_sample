...
function loadPreviewInfoOfPartner(universityId,url) {
	 var params = {};
	 params['university_id']=universityId;
    $('#partnerPagePreview').load(url, params, function(responseText, status, xhr){
        if(status !== 'success') {
            // TODO error handling.
        }
    });
...
function loadTaskView(id) {
	 var params = {};
	 params['id']=id;
    $('#taskPreview').load('?r=consultant/tasks/view', params, function(responseText, status, xhr){
        if(status !== 'success') {
            // TODO error handling.
        }
    });
...
function assignConsultant(id) {
	...
	$.ajax({
            url: '?r=agency/students/assign-consultant&id='+id,
            method: 'POST',
            success: function( data) {  
...
function pendingInvoice(id,task_id){
    var url = "?r=consultant/invoice/create";
    $.ajax({
            url: url,
            method: 'POST',
...
function loadInvoiceView(id) {
     var params = {};
     params['id']=id;
    $('#taskPreview').load('?r=consultant/invoice/view', params, function(responseText, status, xhr){
        if(status !== 'success') {
            // TODO error handling.
        }
    });
...
function loadFollowupView(id) {
     var params = {};
     params['id']=id;
    $('#taskPreview').load('?r=consultant/lead-followup/view', params, function(responseText, status, xhr){
        if(status !== 'success') {
            // TODO error handling.
        }
    });
...
function loadEmailenquiryupdate(id) {
    $.ajax({
            url: '?r=consultant/emailenquiry/update&id='+id,
            method: 'GET', 
            success: function( data) {  
                $('#invoiceUpdate').html(data); 
            },
...
window.onload = function() {
	...
	if ($('.student-profile-tests').length > 0) {  
        $('.btn-update-tests').click(onBtnUpdateClick);
    }
	
	if ($('.student-profile-universities').length > 0) { 
        $('.btn-update-universities').click(onBtnUpdateClick);
    }
	if ($('.student-profile-programs').length > 0) { 
        $('.btn-update-programs').click(onBtnUpdateClick);
    }
